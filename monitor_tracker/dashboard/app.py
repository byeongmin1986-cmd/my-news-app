...
with col_btn:
    if st.button("▶ 지금 전체 크롤링 실행", type="primary", use_container_width=True):
        ...
        if ok:
            st.success(msg)
            st.info("5~10분 후 아래 **🔄 새로고침** 버튼을 눌러 결과를 확인하세요.")
            st.cache_data.clear()
        else:
            st.error(msg)
...
if "_error" in status:
    ...
    if kind == "success":
        ...
        st.error(
            f"**{icon} {label}**\n\n"
            "확인 사항:\n"
            "1. SCRAPERAPI_KEY가 GitHub Secrets에 올바르게 설정됐는지 위 섹션에서 확인\n"
    ...
    for site_key, info in sites.items():
        ...
        if s_status == "success":
            ...
            with st.expander(f"❌ **{s_label}** — 오류 발생 ({s_time} UTC)"):
...
# ══════════════════════════════════════════════════
# 섹션 4: GitHub Actions 최근 실행 내역
# ══════════════════════════════════════════════════
st.subheader("⚡ GitHub Actions 최근 실행 내역")
...
if not token:
    ...
    # "success"인데 실제로 0개 수집이면 failed로 표시
    ...
    run_icon  = ICON_MAP.get(raw_status, "❓")
