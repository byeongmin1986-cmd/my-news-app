...
(function () {
  ...
  function startNewGame() {
    ...
    if (!canvas._gameInited) {
      Game.init(canvas, { onStageClear: handleStageClear, onGameOver: handleGameOver });
  ...
  function resetHUD() {
    ...
    document.querySelectorAll('.life-icon').forEach(el => el.classList.remove('lost'));
  }
  ...
  function nextStage() {
    hideOverlays();
    gameStage++;
    if (gameStage >= Game.getTotalStages()) {
      gameStage = 0;
      gameScore += 500;
  ...
  function renderLeaderboard(rows) {
    ...
    rows.forEach((row, i) => {
      ...
      item.className = 'rank-item' + (isMe ? ' mine' : '') + (rank <= 3 ? ' top' + rank : '');
      ...
      item.innerHTML = numHtml + avatarHtml +
        '<div class="rank-info"><div class="rank-name">' + escapeHtml(row.name || 'Guest') + '</div>' +
        '<div class="rank-stage">스테이지 ' + (row.stage || 1) + '</div></div>' +
        '<div class="rank-score">' + (row.score || 0).toLocaleString() + '</div>';
      listEl.appendChild(item);
    });
    if (rows.length === 0) listEl.innerHTML = '<div class="loading-spinner">아직 기록이 없습니다.</div>';
  ...
  function bindEvents() {
    ...
    $('btn-guest-play').addEventListener('click', () => { currentUser = null; showScreen('game'); });
    $('btn-menu').addEventListener('click', () => { Game.stop(); showOverlay('menu'); });
    $('btn-resume').addEventListener('click', () => {
      hideOverlays();
      const s = Game.getState();
      Game.start(s.stageIndex, s.score, s.lives);
    });
    $('btn-menu-rank').addEventListener('click', () => { hideOverlays(); Game.stop(); showScreen('leaderboard'); });
    $('btn-menu-restart').addEventListener('click', () => { hideOverlays(); startNewGame(); });
    ...
    $('btn-restart').addEventListener('click', () => { hideOverlays(); startNewGame(); });
    $('btn-go-rank').addEventListener('click', () => { hideOverlays(); showScreen('leaderboard'); });
    $('btn-back-game').addEventListener('click', () => {
      ...
      const s = Game.getState();
      if (s && s.phase !== 'cleared') Game.start(s.stageIndex, s.score, s.lives);
      else startNewGame();
    });
